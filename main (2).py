...
tupple_id=[]#declzring a tupple id as a list to store all the unique google ids for the data 
with open('details.csv') as csv_file: #automating the lookup of all professors in the csv file
  
    csv_reader = csv.reader(csv_file, delimiter=',')
  
    for line in csv_reader:
      tupple_id.append(line[1])
print(len(tupple_id))
tupple_id.pop(0)
tupple_id=tuple(tupple_id)#converting the csv into tupple since  it has much faster lookuptime and it dosent allow duplicates
print(tupple_id)
...
#for div in name1:
  ...
  ##add ur code here
    ...
    journal=(item.select(".gs_gray")[1].get_text()).replace(year,'').replace(',','')#adding journal name
